	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else if(flag)
		{
			for(int i = 0;i<cookies.length;i++)
			{
				...
				if("bir_y".equals(cookies[i].getName()))
				{
					bir_y=cookies[i].getValue();
				}
				if("bir_m".equals(cookies[i].getName()))
				{
					bir_m=cookies[i].getValue();
				}
				if("bir_d".equals(cookies[i].getName()))
				{
					bir_d=cookies[i].getValue();
				}
				if("sex".equals(cookies[i].getName()))
				{
					sex=cookies[i].getValue();
				}
				if("school".equals(cookies[i].getName()))
				{
					school=cookies[i].getValue();
				}
				if("information".equals(cookies[i].getName()))
				{
					information=cookies[i].getValue();
				}
			}
		}
